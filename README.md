...
| `analysis_protrusion_per_condition.ipynb` | The full biology pipeline: pool per-condition protrusion µm / sickle cell, sickle fraction, per-FOV stats + bootstrap CIs | Yes for the pool stats (committed parquets); full sweeps need per-condition images (Google Drive) |
